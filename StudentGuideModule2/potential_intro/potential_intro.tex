...
\item Suppose a ball moves along each of the four numbered paths shown below.  For each one, is the work done by the force of gravity \textit{positive}, \textit{negative}, or \textit{zero}? \label{part_potential_intro_grav_work} 
\begin{center}
\vspace{0.1in}
...
\item For each of the four paths in the figure in part \ref{part_potential_intro_grav_work}, state whether the change in potential energy $\Delta U$ of the system is \textit{positive}, \textit{negative}, or \textit{zero}.
\begin{center}
\vspace{0.1in}
...
It's easy to make a careless algebraic mistake with a sign, so always check yourself by telling yourself a little story like this: if you were to reach into the system with your hand and lift a ball along path 2, the external force of your hand would be doing \textit{positive} work on the system, which makes you tired.  The energy you burned went someplace, and it went to \textit{increasing} the gravitational potential energy of the system. 
...
(Hints: this is your old friend, Hooke's law.  Also, if the mass is at a positive $x$, is the force of the spring pushing it in the positive direction or the negative direction?)
%\end{enumerate}
%\end{minipage}
...
\hspace{0.5in}
\raisebox{0.5in}{\includegraphics[scale=0.8]{potential_intro/activity_1_figs/mass_on_spring.eps}}
\end{center}
...
\item The figure below shows a region of uniform electric field $E_0$.  If a \textit{positively} charged particle $+q$ moves along the path shown, does the potential energy of the system \textit{increase}, \textit{decrease}, or \textit{stay the same?}
\begin{center}
%\vspace{0.1in}
...
\item What is the electric force $\vv{F}$ on the charged particle?
\answerspace{0.3in}
...
But the size of the force depends on what charge $q$ you pull from your pocket, and you might not want to recalculate the force from scratch for every possible charge you could pull out.  Instead, you can calculate the electric field $\vv{E}$ due to the fixed charges, and find the force on any charge $q$ using $\vv{F}=q\vv{E}$.  The electric field $\vv{E}$ is telling you the ``force per unit charge'' on whatever extra $q$ you put down, and you can think of that as the definition of electric field:
...
\item Note that the name ``electric potential'' makes it sound like $V$ is an energy, but it's not.  What are the SI units for potential energy $U$?
\answerspace{0.3in}

\item What would be the units of electric potential $V$ in terms of SI units you already know?  (This unit is also defined as the Volt, and is abbreviated as ``V''.)
\answerspace{0.3in}
...
\item What is the final speed $v_F$ of the electron?
\answerspace{0.6in}
...
\item The diagram below shows the relationships between $\vv{F}$, $\vv{E}$, $U$, and $V$ .  Fill in the equations between $\vv{F}$ and $\vv{E}$ and between $U$ and $V$ from memory if you can.  To find the two relationships between $\vv{E}$ and $V$, start with the analogous equations relating $\vv{F}$ and $U$, and divide both sides by $q$.  
\begin{center}
...
\item The figure below shows a region of space with a uniform electric field.  Draw two different dotted line paths, each of which have constant electric potential along the line.  Which of your two paths is at a higher electric potential?  (Each of these paths is called an ``equipotential''.)
\begin{center}
\vspace{-0.1 in}
...
\item The electric field gets stronger as you get closer.  Bearing in mind that $E = -dU/ds$, is the magnitude $\left | {dU}/{dr}\right |$ getting bigger or smaller as you get closer to $Q$?
\answerspace{0.4in}

\item Based on your last two answers, sketch a graph of $V(r)$ on the axes below.  For convenience, let $V=0$ at $r \rightarrow \infty$. \label{part_potential_intro_sketch_of_Vr}
\begin{center}
...
the one-dimensional form $V=-\int{E \, dr}$.  Perform that integration using the $E(r)$ you just wrote. Be sure to include an integration constant $+C$.
\answerspace{1.2in}

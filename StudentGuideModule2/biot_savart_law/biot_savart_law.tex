...
Find the magnetic field $\vec{B}$ at a point $P$ which is a distance $y$ away from the midpoint of a thin wire of length $L$ carrying current $I$.
\begin{center}
    \includegraphics[width=0.7\textwidth]{biot_savart_law/straight_wire.eps}
\end{center}

\textbf{Introduction and outline of solution:}
We’ll divide the wire into a bunch of short lengths $\vec{ds}$, and calculate the field at $P$ using the Biot-Savart law:
\begin{displaymath}
\vec{B} = \int{\frac{\mu_0I}{4 \pi}\frac{\vec{ds}\times \hat{r}}{r^2}}
\end{displaymath}
...
\textbf{Step 2:} \newline
What is the direction of the vectors $\vec{r}$ and $\hat{r}$?
...
\textbf{Step 3:} \newline
What is the direction of  $\vec{ds}\times \hat{r}$?  Is the direction the same for all $\vec{ds}$ along the wire? 

\vspace{.6in}


\textbf{Step 4:} \newline
You know that $\left|\vec{ds}\times \hat{r}\right|=\left|\vec{ds}\right|  \left|\hat{r}\right|\sin\theta$.  Which $\theta$ is the one you want, $\theta_{in}$ or $\theta_{out}$?

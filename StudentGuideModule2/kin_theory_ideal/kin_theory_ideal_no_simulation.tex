...
The first two activities in this lab mostly keeps the structure of deriving a relationship between $P$, $V$, and $\left<  E_{\rm kin} \right>$, starting with a single atom in an $\ell \times \ell \times \ell$ box.  This lab adds a new third activity, which introduces the equipartition theorem and derives the ideal gas law.

I used this in Spring and it went smoothly.

I also decided to use it again in Fall 2020 instead of the other version of ``Kinetic Theory of Ideal Gases.''  I decided I really like the new equipartition theorem activity, and I'm also not crazy about the simulation software in the original version.  However, this adds \textit{yet another} theory-only lab to the manual, which is a bummer.  Perhaps in the future, I or somebody else can recombine the two versions of this lab to retain the best of each version. 
}

...
(c) Check the circuit to make sure that it corresponds to the one
...
The computer and the circuit are setup to measure the voltage
across each of the circuit elements. The resistor in the circuit 
has a value $R=10\Omega$. Record this value
...
below. Use the value for r measured with the DMM. Record the value
of the capacitor (100 \( \mu  \)F) in your data table.
\vspace{15mm}
...
Examine
the phase relationships between the signals. Do these relationships
...
figure above. Choose some scale (for example
1.00 V/cm) so that the diagrams are as large as possible, but so that
they both fit in the space below. Are the phase relationships consistent
with what was observed on the computer screen? Are the phase angles
\( \phi  \) consistent with the calculated values?
\vspace{3.5in}
...
(b) Set the frequency  the generator to 30 Hz and record this
...
between the signals at resonance. Compare them with the predictions

(* Content-type: application/vnd.wolfram.cdf.text *)

(*** Wolfram CDF File ***)
(* http://www.wolfram.com/cdf *)

(* CreatedBy='Mathematica 10.2' *)

(*************************************************************************)
(*                                                                       *)
(*  The Mathematica License under which this file was created prohibits  *)
(*  restricting third parties in receipt of this file from republishing  *)
(*  or redistributing it by any means, including but not limited to      *)
(*  rights management or terms of use, without the express consent of    *)
(*  Wolfram Research, Inc. For additional information concerning CDF     *)
(*  licensing and redistribution see:                                    *)
(*                                                                       *)
(*        www.wolfram.com/cdf/adopting-cdf/licensing-options.html        *)
(*                                                                       *)
(*************************************************************************)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[      1064,         20]
NotebookDataLength[     50981,       1124]
NotebookOptionsPosition[     51119,       1105]
NotebookOutlinePosition[     51538,       1121]
CellTagsIndexPosition[     51495,       1118]
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{

Cell[CellGroupData[{
Cell[BoxData[
 RowBox[{
  RowBox[{"Inten", "[", 
   RowBox[{"t_", ",", "L_", ",", "N_", ",", "D_"}], "]"}], " ", "=", 
  RowBox[{"6", "*", 
   RowBox[{
    RowBox[{"(", 
     RowBox[{
      RowBox[{"Sin", "[", " ", 
       RowBox[{"N", " ", "*", 
        RowBox[{
         RowBox[{"(", 
          RowBox[{"2", "*", "Pi", "*", "D", " ", 
           RowBox[{
            RowBox[{"Sin", "[", "t", "]"}], "/", "L"}]}], ")"}], "/", "2"}]}],
        "]"}], "/", 
      RowBox[{"Sin", "[", 
       RowBox[{
        RowBox[{"(", 
         RowBox[{"2", "*", "Pi", "*", "D", " ", 
          RowBox[{
           RowBox[{"Sin", "[", "t", "]"}], "/", "L"}]}], ")"}], "/", "2"}], 
       "]"}]}], ")"}], "^", "2"}]}]}]], "Input",
 CellOpen->False,
 InitializationCell->True,
 CellChangeTimes->{{3.655988388292108*^9, 3.655988489607903*^9}, {
   3.655988545224084*^9, 3.655988547002186*^9}, {3.656017799270376*^9, 
   3.6560178255440035`*^9}, 3.656017944363884*^9, {3.6560179762290697`*^9, 
   3.6560181333488274`*^9}, 3.6560182005398273`*^9, 3.6560182630358276`*^9, {
   3.6560183248958273`*^9, 3.6560183373448277`*^9}, {3.6560183786658278`*^9, 
   3.6560184015968275`*^9}, {3.656018492399828*^9, 3.6560184975658274`*^9}, {
   3.656018834797928*^9, 3.656018873812829*^9}, {3.6560189491073575`*^9, 
   3.656018956110058*^9}, {3.6560189926107073`*^9, 3.6560189984412904`*^9}, {
   3.656019113154761*^9, 3.6560191277852235`*^9}, {3.656021792000991*^9, 
   3.6560218161744084`*^9}, {3.6560225956593494`*^9, 3.656022604787262*^9}}],

Cell[BoxData[
 RowBox[{"6", " ", 
  SuperscriptBox[
   RowBox[{"Csc", "[", 
    FractionBox[
     RowBox[{"D", " ", "\[Pi]", " ", 
      RowBox[{"Sin", "[", "t", "]"}]}], "L"], "]"}], "2"], " ", 
  SuperscriptBox[
   RowBox[{"Sin", "[", 
    FractionBox[
     RowBox[{"D", " ", "N", " ", "\[Pi]", " ", 
      RowBox[{"Sin", "[", "t", "]"}]}], "L"], "]"}], "2"]}]], "Output",
 CellOpen->False,
 CellChangeTimes->{3.656158078497754*^9, 3.781206488350071*^9, 
  3.7951180582275143`*^9}]
}, {2}]],

Cell[CellGroupData[{

Cell[BoxData[
 RowBox[{
  RowBox[{"Fraunhofer", "[", 
   RowBox[{"t_", ",", "L_", ",", "a_"}], "]"}], "=", 
  RowBox[{"15", "*", "a", "*", 
   RowBox[{
    RowBox[{"(", 
     RowBox[{
      RowBox[{"Sin", "[", 
       RowBox[{"Pi", " ", "a", " ", 
        RowBox[{
         RowBox[{"Sin", "[", "t", "]"}], "/", "L"}]}], "]"}], "/", 
      RowBox[{"(", 
       RowBox[{"Pi", " ", "a", " ", 
        RowBox[{
         RowBox[{"Sin", "[", "t", "]"}], "/", "L"}]}], ")"}]}], ")"}], "^", 
    "2"}]}]}]], "Input",
 CellOpen->False,
 InitializationCell->True,
 CellChangeTimes->{{3.6560669600917788`*^9, 3.6560669804407434`*^9}, {
   3.656067014026581*^9, 3.6560671450675807`*^9}, 3.6560672155445805`*^9, {
   3.6560942860049224`*^9, 3.656094286309922*^9}, 3.6560967692059927`*^9, {
   3.7951182492300596`*^9, 3.795118250009073*^9}}],

Cell[BoxData[
 FractionBox[
  RowBox[{"15", " ", 
   SuperscriptBox["L", "2"], " ", 
   SuperscriptBox[
    RowBox[{"Csc", "[", "t", "]"}], "2"], " ", 
   SuperscriptBox[
    RowBox[{"Sin", "[", 
     FractionBox[
      RowBox[{"a", " ", "\[Pi]", " ", 
       RowBox[{"Sin", "[", "t", "]"}]}], "L"], "]"}], "2"]}], 
  RowBox[{"a", " ", 
   SuperscriptBox["\[Pi]", "2"]}]]], "Output",
 CellOpen->False,
 CellChangeTimes->{3.6560671472875805`*^9, 3.656067216767581*^9, 
  3.6560942932069225`*^9, 3.656096770576129*^9, 3.656158078700774*^9, 
  3.781206488440727*^9, 3.7951180586106176`*^9, 3.7951182618480406`*^9}]
}, Closed]],

Cell[CellGroupData[{

Cell[BoxData[{
 RowBox[{
  RowBox[{"DarkWidth", "[", 
   RowBox[{"t_", ",", "L_", ",", "a_"}], "]"}], "=", 
  RowBox[{
   RowBox[{"Quotient", "[", 
    RowBox[{"a", " ", ",", 
     RowBox[{"L", "/", 
      RowBox[{"Sin", "[", "t", "]"}]}]}], "]"}], 
   RowBox[{"L", "/", 
    RowBox[{"Sin", "[", "t", "]"}]}]}]}], "\[IndentingNewLine]", 
 RowBox[{
  RowBox[{"DarkWidth2", "[", 
   RowBox[{"t_", ",", "L_", ",", "a_"}], "]"}], "=", 
  RowBox[{"Mod", "[", 
   RowBox[{"a", ",", 
    RowBox[{"L", "/", 
     RowBox[{"(", 
      RowBox[{"2", " ", 
       RowBox[{"Sin", "[", "t", "]"}]}], ")"}]}]}], 
   "]"}]}], "\[IndentingNewLine]", 
 RowBox[{"Manipulate", "[", "\[IndentingNewLine]", 
  RowBox[{
   RowBox[{"Row", "[", 
    RowBox[{"{", 
     RowBox[{
      RowBox[{"Show", "[", "\[IndentingNewLine]", 
       RowBox[{
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{"0", ",", 
              RowBox[{"-", "20"}]}], "}"}], ",", 
            RowBox[{"{", 
             RowBox[{"0", ",", "20"}], "}"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{"Black", ",", "Thick"}], "}"}]}], ",", " ", 
          RowBox[{"Axes", "\[Rule]", "False"}], ",", " ", 
          RowBox[{"PlotRange", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{
             RowBox[{"{", 
              RowBox[{"0", ",", "20"}], "}"}], ",", 
             RowBox[{"{", 
              RowBox[{
               RowBox[{"-", "10"}], ",", "10"}], "}"}]}], "}"}]}], ",", 
          RowBox[{"AspectRatio", "\[Rule]", "1"}], ",", 
          RowBox[{"ImageSize", "\[Rule]", "Large"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"PointSize", "[", "Medium", "]"}]}], ",", 
          RowBox[{"PlotLabel", "\[Rule]", "\"\<Close In View\>\""}]}], "]"}], 
        ",", "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{"0", ",", 
              RowBox[{
               RowBox[{"-", "a"}], "/", "2"}]}], "}"}], ",", 
            RowBox[{"{", 
             RowBox[{"0", ",", 
              RowBox[{"a", "/", "2"}]}], "}"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{"White", ",", "Thick"}], "}"}]}]}], "]"}], ",", 
        RowBox[{"Graphics", "[", 
         RowBox[{"{", 
          RowBox[{
           RowBox[{"RGBColor", "[", 
            RowBox[{"1.0", ",", "0.8", ",", "0.8"}], "]"}], ",", 
           RowBox[{"Polygon", "[", 
            RowBox[{"{", 
             RowBox[{
              RowBox[{"{", 
               RowBox[{"0", ",", 
                RowBox[{
                 RowBox[{"-", "a"}], "/", "2"}]}], "}"}], ",", 
              RowBox[{"{", 
               RowBox[{"0", ",", 
                RowBox[{"a", "/", "2"}]}], "}"}], ",", 
              RowBox[{"{", 
               RowBox[{"30", ",", 
                RowBox[{
                 RowBox[{"30", 
                  RowBox[{"Tan", "[", "t", "]"}]}], "+", 
                 RowBox[{"a", "/", "2"}]}]}], "}"}], ",", 
              RowBox[{"{", 
               RowBox[{"30", ",", 
                RowBox[{
                 RowBox[{"30", 
                  RowBox[{"Tan", "[", "t", "]"}]}], "-", 
                 RowBox[{"a", "/", "2"}]}]}], "}"}]}], "}"}], "]"}]}], "}"}], 
         "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"If", "[", 
         RowBox[{
          RowBox[{
           RowBox[{"a", " ", 
            RowBox[{"Sin", "[", "t", "]"}]}], " ", ">", " ", "L"}], ",", 
          "\[IndentingNewLine]", 
          RowBox[{"Graphics", "[", 
           RowBox[{"{", 
            RowBox[{
             RowBox[{"RGBColor", "[", 
              RowBox[{"0.8", ",", "0.6", ",", "0.6"}], "]"}], ",", 
             RowBox[{"Polygon", "[", 
              RowBox[{"{", 
               RowBox[{
                RowBox[{"{", 
                 RowBox[{"0", ",", 
                  RowBox[{"a", "/", "2"}]}], "}"}], ",", 
                RowBox[{"{", 
                 RowBox[{"30", ",", 
                  RowBox[{
                   RowBox[{"30", 
                    RowBox[{"Tan", "[", "t", "]"}]}], "+", 
                   RowBox[{"a", "/", "2"}]}]}], "}"}], ",", 
                RowBox[{"{", 
                 RowBox[{"30", ",", 
                  RowBox[{
                   RowBox[{"30", 
                    RowBox[{"Tan", "[", "t", "]"}]}], "+", 
                   RowBox[{"a", "/", "2"}], " ", "-", 
                   RowBox[{"DarkWidth", "[", 
                    RowBox[{"t", ",", "L", ",", "a"}], "]"}]}]}], " ", "}"}], 
                ",", 
                RowBox[{"{", 
                 RowBox[{"0", ",", 
                  RowBox[{
                   RowBox[{"a", "/", "2"}], " ", "-", 
                   RowBox[{"DarkWidth", "[", 
                    RowBox[{"t", ",", "L", ",", "a"}], "]"}]}]}], "}"}]}], 
               "}"}], "]"}]}], "}"}], "]"}], ",", 
          RowBox[{"ListPlot", "[", 
           RowBox[{"{", 
            RowBox[{"{", 
             RowBox[{
              RowBox[{"-", "10"}], ",", " ", "0"}], "}"}], "}"}], "]"}]}], 
         "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"If", "[", 
         RowBox[{
          RowBox[{
           RowBox[{"Mod", "[", 
            RowBox[{
             RowBox[{"a", " ", 
              RowBox[{"Sin", "[", "t", "]"}]}], ",", "L"}], "]"}], " ", ">", 
           " ", 
           RowBox[{"L", "/", "2"}]}], ",", "\[IndentingNewLine]", 
          RowBox[{"Graphics", "[", "\[IndentingNewLine]", 
           RowBox[{"{", 
            RowBox[{
             RowBox[{"RGBColor", "[", 
              RowBox[{"0.8", ",", "0.6", ",", "0.6"}], "]"}], ",", 
             RowBox[{"Polygon", "[", 
              RowBox[{"{", 
               RowBox[{
                RowBox[{"{", 
                 RowBox[{"0", ",", 
                  RowBox[{
                   RowBox[{"-", "a"}], "/", "2"}]}], "}"}], ",", 
                RowBox[{"{", 
                 RowBox[{"30", ",", 
                  RowBox[{
                   RowBox[{"30", 
                    RowBox[{"Tan", "[", "t", "]"}]}], "-", 
                   RowBox[{"a", "/", "2"}]}]}], "}"}], ",", 
                RowBox[{"{", 
                 RowBox[{"30", ",", 
                  RowBox[{
                   RowBox[{"30", 
                    RowBox[{"Tan", "[", "t", "]"}]}], "-", 
                   RowBox[{"a", "/", "2"}], " ", "+", 
                   RowBox[{"DarkWidth2", "[", 
                    RowBox[{"t", ",", "L", ",", "a"}], "]"}]}]}], " ", "}"}], 
                ",", 
                RowBox[{"{", 
                 RowBox[{"0", ",", 
                  RowBox[{
                   RowBox[{
                    RowBox[{"-", "a"}], "/", "2"}], " ", "+", 
                   RowBox[{"DarkWidth2", "[", 
                    RowBox[{"t", ",", "L", ",", "a"}], "]"}]}]}], "}"}]}], 
               "}"}], "]"}]}], "}"}], "]"}], ",", 
          RowBox[{"ListPlot", "[", 
           RowBox[{"{", 
            RowBox[{"{", 
             RowBox[{
              RowBox[{"-", "10"}], ",", " ", "0"}], "}"}], "}"}], "]"}]}], 
         "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"If", "[", 
         RowBox[{
          RowBox[{
           RowBox[{"Mod", "[", 
            RowBox[{
             RowBox[{"a", " ", 
              RowBox[{"Sin", "[", "t", "]"}]}], ",", "L"}], "]"}], " ", ">", 
           " ", 
           RowBox[{"L", "/", "2"}]}], ",", "\[IndentingNewLine]", 
          RowBox[{"Graphics", "[", "\[IndentingNewLine]", 
           RowBox[{"{", 
            RowBox[{
             RowBox[{"RGBColor", "[", 
              RowBox[{"0.8", ",", "0.6", ",", "0.6"}], "]"}], ",", 
             RowBox[{"Polygon", "[", 
              RowBox[{"{", 
               RowBox[{
                RowBox[{"{", 
                 RowBox[{"0", ",", 
                  RowBox[{
                   RowBox[{
                    RowBox[{"-", "a"}], "/", "2"}], "+", 
                   RowBox[{"L", "/", 
                    RowBox[{"(", 
                    RowBox[{"2", " ", 
                    RowBox[{"Sin", "[", "t", "]"}]}], ")"}]}]}]}], "}"}], ",", 
                RowBox[{"{", 
                 RowBox[{"30", ",", 
                  RowBox[{
                   RowBox[{"30", 
                    RowBox[{"Tan", "[", "t", "]"}]}], "-", 
                   RowBox[{"a", "/", "2"}], "+", 
                   RowBox[{"L", "/", 
                    RowBox[{"(", 
                    RowBox[{"2", " ", 
                    RowBox[{"Sin", "[", "t", "]"}]}], ")"}]}]}]}], "}"}], ",", 
                RowBox[{"{", 
                 RowBox[{"30", ",", 
                  RowBox[{
                   RowBox[{"30", 
                    RowBox[{"Tan", "[", "t", "]"}]}], "-", 
                   RowBox[{"a", "/", "2"}], " ", "+", 
                   RowBox[{"L", "/", 
                    RowBox[{"(", 
                    RowBox[{"2", " ", 
                    RowBox[{"Sin", "[", "t", "]"}]}], ")"}]}], "+", 
                   RowBox[{"DarkWidth2", "[", 
                    RowBox[{"t", ",", "L", ",", "a"}], "]"}]}]}], " ", "}"}], 
                ",", 
                RowBox[{"{", 
                 RowBox[{"0", ",", 
                  RowBox[{
                   RowBox[{
                    RowBox[{"-", "a"}], "/", "2"}], " ", "+", 
                   RowBox[{"L", "/", 
                    RowBox[{"(", 
                    RowBox[{"2", " ", 
                    RowBox[{"Sin", "[", "t", "]"}]}], ")"}]}], "+", 
                   RowBox[{"DarkWidth2", "[", 
                    RowBox[{"t", ",", "L", ",", "a"}], "]"}]}]}], "}"}]}], 
               "}"}], "]"}]}], "}"}], "]"}], ",", 
          RowBox[{"ListPlot", "[", 
           RowBox[{"{", 
            RowBox[{"{", 
             RowBox[{
              RowBox[{"-", "10"}], ",", " ", "0"}], "}"}], "}"}], "]"}]}], 
         "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"Table", "[", "\[IndentingNewLine]", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{
              RowBox[{"{", 
               RowBox[{"0", ",", "yoff"}], "}"}], ",", 
              RowBox[{
               RowBox[{"RotationTransform", "[", 
                RowBox[{"t", ",", 
                 RowBox[{"{", 
                  RowBox[{"0", ",", "yoff"}], "}"}]}], "]"}], "[", 
               RowBox[{"{", 
                RowBox[{"30", ",", "yoff"}], "}"}], "]"}]}], "}"}], ",", 
            RowBox[{"{", 
             RowBox[{"yoff", ",", 
              RowBox[{"a", "/", "2"}], ",", 
              RowBox[{
               RowBox[{"-", "a"}], "/", "2"}], ",", 
              RowBox[{
               RowBox[{"-", "L"}], "/", 
               RowBox[{"(", 
                RowBox[{"2", 
                 RowBox[{"Sin", "[", "t", "]"}]}], ")"}]}]}], "}"}]}], "]"}], 
          ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", "Red", "}"}]}]}], "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"Table", "[", 
         RowBox[{
          RowBox[{"ParametricPlot", "[", "\[IndentingNewLine]", 
           RowBox[{
            RowBox[{
             RowBox[{"RotationTransform", "[", 
              RowBox[{"t", ",", 
               RowBox[{"{", 
                RowBox[{"0", ",", "yoff"}], "}"}]}], "]"}], "[", 
             RowBox[{"{", 
              RowBox[{"u", ",", 
               RowBox[{
                RowBox[{".4", 
                 RowBox[{"Sin", "[", 
                  RowBox[{"2", "Pi", " ", 
                   RowBox[{"u", "/", "L"}]}], "]"}]}], "+", "yoff"}]}], "}"}],
              "]"}], ",", 
            RowBox[{"{", 
             RowBox[{"u", ",", "0", ",", "5"}], "}"}], ",", 
            RowBox[{"PlotStyle", "\[Rule]", 
             RowBox[{"{", "Red", "}"}]}], ",", 
            RowBox[{"PlotPoints", "\[Rule]", 
             RowBox[{"Ceiling", "[", 
              RowBox[{"20", "/", "L"}], "]"}]}], ",", 
            RowBox[{"MaxRecursion", "\[Rule]", "1"}], ",", 
            RowBox[{"PerformanceGoal", "\[Rule]", 
             RowBox[{"{", "\"\<Speed\>\"", "}"}]}]}], "]"}], ",", 
          RowBox[{"{", 
           RowBox[{"yoff", ",", 
            RowBox[{"a", "/", "2"}], ",", 
            RowBox[{
             RowBox[{"-", "a"}], "/", "2"}], ",", 
            RowBox[{
             RowBox[{"-", "L"}], "/", 
             RowBox[{"(", 
              RowBox[{"2", 
               RowBox[{"Sin", "[", "t", "]"}]}], ")"}]}]}], "}"}]}], "]"}], 
        ",", "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{"0", ",", 
              RowBox[{"a", "/", "2"}]}], "}"}], ",", 
            RowBox[{
             RowBox[{"RotationTransform", "[", 
              RowBox[{"t", ",", 
               RowBox[{"{", 
                RowBox[{"0", ",", 
                 RowBox[{"a", "/", "2"}]}], "}"}]}], "]"}], "[", 
             RowBox[{"{", 
              RowBox[{"0", ",", 
               RowBox[{
                RowBox[{"-", "a"}], "/", "2"}]}], "}"}], "]"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{"Dashed", ",", " ", 
             RowBox[{"PointSize", "[", "Large", "]"}], ",", "Black"}], 
            "}"}]}]}], "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{"0", ",", 
              RowBox[{
               RowBox[{"-", "a"}], "/", "2"}]}], "}"}], ",", 
            RowBox[{
             RowBox[{"RotationTransform", "[", 
              RowBox[{"t", ",", 
               RowBox[{"{", 
                RowBox[{"0", ",", 
                 RowBox[{
                  RowBox[{"-", "a"}], "/", "2"}]}], "}"}]}], "]"}], "[", 
             RowBox[{"{", 
              RowBox[{"0", ",", 
               RowBox[{
                RowBox[{"+", "a"}], "/", "2"}]}], "}"}], "]"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{"Dashed", ",", " ", 
             RowBox[{"PointSize", "[", "Large", "]"}], ",", "Black"}], 
            "}"}]}]}], "]"}]}], "\[IndentingNewLine]", "]"}], 
      "\[IndentingNewLine]", ",", "\[IndentingNewLine]", 
      RowBox[{"Show", "[", "\[IndentingNewLine]", 
       RowBox[{
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{"0", ",", 
              RowBox[{"-", "30"}]}], "}"}], ",", 
            RowBox[{"{", 
             RowBox[{"0", ",", "30"}], "}"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{"Black", ",", "Thicker"}], "}"}]}], ",", " ", 
          RowBox[{"Axes", "\[Rule]", "False"}], ",", " ", 
          RowBox[{"PlotRange", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{
             RowBox[{"{", 
              RowBox[{
               RowBox[{"-", "10"}], ",", "190"}], "}"}], ",", 
             RowBox[{"{", 
              RowBox[{
               RowBox[{"-", "105"}], ",", "95"}], "}"}]}], "}"}]}], ",", 
          RowBox[{"AspectRatio", "\[Rule]", "1"}], ",", 
          RowBox[{"ImageSize", "\[Rule]", "Large"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"PointSize", "[", "Medium", "]"}]}], ",", " ", 
          RowBox[{"PlotLabel", "\[Rule]", "\"\<Far Out View\>\""}]}], "]"}], 
        ",", "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{"0", ",", 
              RowBox[{
               RowBox[{"-", "a"}], "/", "2"}]}], "}"}], ",", 
            RowBox[{"{", 
             RowBox[{"0", ",", 
              RowBox[{"a", "/", "2"}]}], "}"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{"White", ",", "Thick"}], "}"}]}]}], "]"}], ",", 
        "\[IndentingNewLine]", 
        RowBox[{"Graphics", "[", 
         RowBox[{"{", 
          RowBox[{
           RowBox[{"RGBColor", "[", 
            RowBox[{
             RowBox[{"{", 
              RowBox[{"0.8", ",", "0.6", ",", "0.6"}], "}"}], "+", 
             RowBox[{"(", 
              RowBox[{
               RowBox[{"{", 
                RowBox[{"0.2", ",", "0.2", ",", "0.2"}], "}"}], "*", 
               RowBox[{
                RowBox[{"Fraunhofer", "[", 
                 RowBox[{"t", ",", "L", ",", "a"}], "]"}], "/", 
                RowBox[{"(", 
                 RowBox[{"15", "a"}], ")"}]}]}], ")"}]}], "]"}], ",", 
           RowBox[{"Polygon", "[", 
            RowBox[{"{", 
             RowBox[{
              RowBox[{"{", 
               RowBox[{"0", ",", 
                RowBox[{
                 RowBox[{"-", "a"}], "/", "2"}]}], "}"}], ",", 
              RowBox[{"{", 
               RowBox[{"0", ",", 
                RowBox[{"a", "/", "2"}]}], "}"}], ",", 
              RowBox[{"{", 
               RowBox[{"190", ",", 
                RowBox[{
                 RowBox[{"190", 
                  RowBox[{"Tan", "[", "t", "]"}]}], "+", "0.5"}]}], "}"}], 
              ",", 
              RowBox[{"{", 
               RowBox[{"190", ",", 
                RowBox[{
                 RowBox[{"190", 
                  RowBox[{"Tan", "[", "t", "]"}]}], "-", "0.5"}]}], "}"}]}], 
             "}"}], "]"}]}], "}"}], "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"ParametricPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"190", "-", 
             RowBox[{"Fraunhofer", "[", 
              RowBox[{"tt", ",", "L", ",", "a"}], "]"}]}], ",", 
            RowBox[{"190", 
             RowBox[{"Tan", "[", "tt", "]"}]}]}], "}"}], ",", 
          RowBox[{"{", 
           RowBox[{"tt", ",", 
            RowBox[{"-", "0.6"}], ",", ".6"}], "}"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", "Blue", "}"}]}], ",", 
          RowBox[{"PlotPoints", "\[Rule]", "100"}], ",", 
          RowBox[{"MaxRecursion", "\[Rule]", "1"}], ",", 
          RowBox[{"PerformanceGoal", "\[Rule]", 
           RowBox[{"{", "\"\<Speed\>\"", "}"}]}]}], "]"}], ",", 
        "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{
              RowBox[{"190", "-", 
               RowBox[{"Fraunhofer", "[", 
                RowBox[{"t", ",", "L", ",", "a"}], "]"}]}], ",", 
              RowBox[{"190", 
               RowBox[{"Tan", "[", "t", "]"}]}]}], "}"}], ",", 
            RowBox[{"{", 
             RowBox[{"190", ",", 
              RowBox[{"190", 
               RowBox[{"Tan", "[", "t", "]"}]}]}], "}"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{"Dashed", ",", " ", "Blue"}], "}"}]}]}], "]"}], ",", 
        "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{"{", 
            RowBox[{
             RowBox[{"190", "-", 
              RowBox[{"Fraunhofer", "[", 
               RowBox[{"t", ",", "L", ",", "a"}], "]"}]}], ",", 
             RowBox[{"190", 
              RowBox[{"Tan", "[", "t", "]"}]}]}], "}"}], "}"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{
             RowBox[{"PointSize", "[", "Large", "]"}], ",", " ", "Blue"}], 
            "}"}]}]}], "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{"0", ",", "0"}], "}"}], ",", 
            RowBox[{"{", 
             RowBox[{"200", ",", "0"}], "}"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", 
            RowBox[{"Dashed", ",", " ", 
             RowBox[{"PointSize", "[", "Small", "]"}], ",", "Black"}], 
            "}"}]}]}], "]"}], ",", "\[IndentingNewLine]", 
        RowBox[{"ListPlot", "[", 
         RowBox[{
          RowBox[{"{", 
           RowBox[{
            RowBox[{"{", 
             RowBox[{"189.8", ",", 
              RowBox[{"-", "100"}]}], "}"}], ",", 
            RowBox[{"{", 
             RowBox[{"189.8", ",", "1000"}], "}"}]}], "}"}], ",", 
          RowBox[{"Joined", "\[Rule]", "True"}], ",", 
          RowBox[{"PlotStyle", "\[Rule]", 
           RowBox[{"{", "Black", "}"}]}]}], "\[IndentingNewLine]", "]"}]}], 
       "]"}]}], "}"}], "]"}], "\[IndentingNewLine]", ",", " ", 
   RowBox[{"{", 
    RowBox[{
     RowBox[{"{", 
      RowBox[{"t", ",", ".115", ",", "\"\<Angle\>\""}], "}"}], ",", "0.0001", 
     ",", ".5235"}], "}"}], ",", " ", 
   RowBox[{"{", 
    RowBox[{
     RowBox[{"{", 
      RowBox[{"L", ",", "1", ",", "\"\<Wavelength\>\""}], "}"}], ",", "0.5", 
     ",", "2"}], "}"}], ",", 
   RowBox[{"{", 
    RowBox[{
     RowBox[{"{", 
      RowBox[{"a", ",", "10", ",", "\"\<Slit Width\>\""}], "}"}], ",", "4", 
     ",", "10"}], "}"}], ",", 
   RowBox[{"SaveDefinitions", "\[Rule]", "True"}]}], "]"}]}], "Input",
 InitializationCell->True,
 CellChangeTimes->{{3.429610822746764*^9, 3.4296108241686487`*^9}, {
   3.4296109428720207`*^9, 3.42961096165315*^9}, {3.4296110106528606`*^9, 
   3.4296110111997356`*^9}, {3.4296110500591106`*^9, 
   3.4296110584809856`*^9}, {3.429611151996501*^9, 3.4296111699807615`*^9}, {
   3.429611312964631*^9, 3.4296113328551283`*^9}, {3.4614137606928787`*^9, 
   3.4614138478055563`*^9}, {3.461413974981*^9, 3.461414011618601*^9}, {
   3.461414043494417*^9, 3.461414063604307*^9}, {3.4614141165431623`*^9, 
   3.4614141396682844`*^9}, {3.4614141818682566`*^9, 
   3.4614141832154636`*^9}, {3.4614142246969132`*^9, 3.461414225182534*^9}, {
   3.4614142676508913`*^9, 3.4614143297947397`*^9}, {3.4614143838553452`*^9, 
   3.4614144166112785`*^9}, {3.4614144561831026`*^9, 3.461414466443205*^9}, {
   3.4614145070926046`*^9, 3.4614145367468033`*^9}, {3.4614145791479273`*^9, 
   3.461414598635866*^9}, {3.461414736943923*^9, 3.461414737757224*^9}, {
   3.461414777373025*^9, 3.4614147827033668`*^9}, {3.461414876476735*^9, 
   3.4614148816663933`*^9}, {3.461414952367668*^9, 3.461414979519583*^9}, {
   3.5559507062388077`*^9, 3.555950710122958*^9}, {3.5559507951843057`*^9, 
   3.555950802578232*^9}, {3.5559508622614975`*^9, 3.55595090998098*^9}, {
   3.5559509585583096`*^9, 3.555950999007294*^9}, {3.55595107077528*^9, 
   3.5559510730371785`*^9}, {3.555951116730818*^9, 3.5559511218161893`*^9}, 
   3.555951233117195*^9, {3.555951264955741*^9, 3.555951266000928*^9}, 
   3.5559513069816027`*^9, {3.555951337292014*^9, 3.555951342439948*^9}, {
   3.555951420017753*^9, 3.5559514234185095`*^9}, {3.5559518150860453`*^9, 
   3.555951837581245*^9}, {3.555951924442045*^9, 3.555951925019245*^9}, {
   3.555952416450445*^9, 3.555952450193245*^9}, 3.555953941628399*^9, {
   3.5560159525817676`*^9, 3.556015992002968*^9}, {3.5874615104521246`*^9, 
   3.5874615504706545`*^9}, {3.587461593417655*^9, 3.5874616223226547`*^9}, {
   3.5874616550396547`*^9, 3.5874616791736546`*^9}, {3.650729579729862*^9, 
   3.6507295846492386`*^9}, {3.6559766964023705`*^9, 
   3.6559766977484474`*^9}, {3.6559767636582174`*^9, 
   3.6559767646322727`*^9}, {3.6559768091338177`*^9, 
   3.6559768132980566`*^9}, {3.655976897528874*^9, 3.655976898963956*^9}, {
   3.6559769614415293`*^9, 3.6559770048630133`*^9}, 3.655977494194001*^9, {
   3.6559775660671124`*^9, 3.6559776645417447`*^9}, {3.655977738375968*^9, 
   3.6559777475064898`*^9}, {3.6559778321343307`*^9, 3.6559778383296847`*^9}, 
   3.655977927201768*^9, 3.655977978041676*^9, {3.655978017479932*^9, 
   3.65597802601742*^9}, {3.6559782166213217`*^9, 3.6559782556945567`*^9}, 
   3.6559784121545057`*^9, {3.65597856747939*^9, 3.6559786088747573`*^9}, {
   3.6559787211091766`*^9, 3.655978755150124*^9}, {3.6559789422688265`*^9, 
   3.6559790311299095`*^9}, {3.6559790682420316`*^9, 
   3.6559791008858986`*^9}, {3.655979148524624*^9, 3.655979193067171*^9}, {
   3.6559792238019295`*^9, 3.655979250856477*^9}, {3.655979928703247*^9, 
   3.655980104376295*^9}, {3.6559801762554064`*^9, 3.655980183957847*^9}, {
   3.6559802287134066`*^9, 3.6559802388129845`*^9}, {3.6559802970833178`*^9, 
   3.655980314700325*^9}, {3.6559803724016256`*^9, 3.655980475062497*^9}, {
   3.6559805129126625`*^9, 3.655980545112504*^9}, {3.6559806133364058`*^9, 
   3.655980700440388*^9}, {3.6559807306841183`*^9, 3.6559807410837126`*^9}, {
   3.655981041610902*^9, 3.655981140992586*^9}, 3.6559811948916693`*^9, {
   3.655981240430274*^9, 3.6559812845677986`*^9}, {3.6559813371478057`*^9, 
   3.6559813635593166`*^9}, 3.6559815468397994`*^9, {3.655981618974925*^9, 
   3.655981657013101*^9}, 3.655981751539508*^9, {3.655981933010887*^9, 
   3.6559821048557158`*^9}, {3.655982144070959*^9, 3.6559821565716743`*^9}, {
   3.6559822751144543`*^9, 3.655982301066939*^9}, {3.655982421239812*^9, 
   3.655982430072317*^9}, {3.6559824693055615`*^9, 3.6559825315811234`*^9}, {
   3.655982602186162*^9, 3.655982668014927*^9}, {3.6559828918927317`*^9, 
   3.655982916934164*^9}, {3.655982965240927*^9, 3.655982989716327*^9}, {
   3.6559830522909064`*^9, 3.6559830907601066`*^9}, {3.6559831624632077`*^9, 
   3.6559832914205837`*^9}, {3.655983376547453*^9, 3.6559834335427127`*^9}, {
   3.655983551427455*^9, 3.655983608125698*^9}, {3.6559836768456287`*^9, 
   3.655983684591072*^9}, 3.6559838268152065`*^9, {3.6559838631112823`*^9, 
   3.6559838632472906`*^9}, {3.6559838985013065`*^9, 3.65598395467752*^9}, {
   3.6559858916523085`*^9, 3.655985917325777*^9}, {3.655985955924985*^9, 
   3.655986009835068*^9}, {3.655986946161623*^9, 3.6559869490397873`*^9}, {
   3.6559870259641876`*^9, 3.6559870415060763`*^9}, {3.6559876463706727`*^9, 
   3.655987663404647*^9}, {3.655987708476225*^9, 3.6559877097412972`*^9}, {
   3.655987750018601*^9, 3.6559877532907877`*^9}, {3.655987878025923*^9, 
   3.6559879534842386`*^9}, {3.655988101368697*^9, 3.655988114283436*^9}, {
   3.6559885074399233`*^9, 3.6559885173034873`*^9}, {3.6559885715855923`*^9, 
   3.655988577591935*^9}, {3.655988758416278*^9, 3.6559888058129888`*^9}, {
   3.6559889259888625`*^9, 3.6559889575806694`*^9}, {3.6559890552542562`*^9, 
   3.655989077070504*^9}, {3.6559974721856775`*^9, 3.6559975684391823`*^9}, {
   3.655997611452643*^9, 3.6559976330338774`*^9}, 3.655997670062995*^9, {
   3.6559977128494425`*^9, 3.655997715874615*^9}, {3.655997803525629*^9, 
   3.6559978229157376`*^9}, {3.655997959098527*^9, 3.6559981650953093`*^9}, 
   3.6559986233585205`*^9, {3.6559986790597067`*^9, 3.6559987032120876`*^9}, {
   3.655998735977962*^9, 3.6559987582412353`*^9}, {3.655998793959278*^9, 
   3.6559988094071617`*^9}, {3.6559988469003067`*^9, 3.655998847907364*^9}, {
   3.655998949879196*^9, 3.655999002905229*^9}, {3.6559991285494156`*^9, 
   3.655999170948841*^9}, {3.6559993447447815`*^9, 3.6559993451138024`*^9}, {
   3.656011178499859*^9, 3.6560112123044662`*^9}, {3.656011270049735*^9, 
   3.6560113178398275`*^9}, {3.656011356310583*^9, 3.6560114088662915`*^9}, 
   3.656011460461481*^9, {3.656011532263562*^9, 3.6560115406423707`*^9}, 
   3.656012106403082*^9, {3.6560121420810823`*^9, 3.656012177948082*^9}, {
   3.6560122269710817`*^9, 3.656012246053082*^9}, {3.6560122965960817`*^9, 
   3.656012525344082*^9}, {3.65601262186829*^9, 3.65601265987209*^9}, {
   3.656013103000481*^9, 3.6560131683994813`*^9}, {3.656013201934481*^9, 
   3.656013268371481*^9}, {3.656013774888481*^9, 3.656013785626481*^9}, 
   3.6560138602944813`*^9, {3.6560139114932537`*^9, 3.6560139293640404`*^9}, {
   3.656014024540557*^9, 3.656014060523155*^9}, {3.65601415158226*^9, 
   3.6560141560177035`*^9}, 3.6560144163227315`*^9, {3.6560145229423923`*^9, 
   3.6560146477428713`*^9}, {3.6560146994030366`*^9, 3.656014700030099*^9}, {
   3.656014880416136*^9, 3.656014946083702*^9}, 3.656015099331025*^9, {
   3.656015241830274*^9, 3.656015248564947*^9}, {3.6560153960556946`*^9, 
   3.656015402041293*^9}, {3.656015453720461*^9, 3.6560154553296213`*^9}, {
   3.656015530893177*^9, 3.656015540800168*^9}, {3.656015785676653*^9, 
   3.6560157891169972`*^9}, {3.6560158419222775`*^9, 
   3.6560158555926437`*^9}, {3.6560158902711115`*^9, 
   3.6560158929693813`*^9}, {3.656016082424004*^9, 3.6560160837898674`*^9}, {
   3.656016145602686*^9, 3.656016159249321*^9}, {3.6560162031469307`*^9, 
   3.656016225980647*^9}, 3.656016263235921*^9, {3.6560162958776565`*^9, 
   3.6560163225429897`*^9}, 3.656016386045666*^9, {3.6560166224316664`*^9, 
   3.656016622916666*^9}, {3.656016654425666*^9, 3.656016656378666*^9}, {
   3.6560167036156664`*^9, 3.6560167044436665`*^9}, {3.6560167830566664`*^9, 
   3.656016795065666*^9}, {3.6560168619656663`*^9, 3.6560168633076663`*^9}, {
   3.6560181617328277`*^9, 3.6560181655418277`*^9}, {3.6560185338918276`*^9, 
   3.6560185791658278`*^9}, 3.656018790175466*^9, 3.656018885607008*^9, {
   3.6560190757130165`*^9, 3.656019095216967*^9}, 3.6560191406455097`*^9, {
   3.656019881387576*^9, 3.656019917361173*^9}, 3.6560199621836553`*^9, {
   3.6560201012545605`*^9, 3.6560201201854534`*^9}, {3.656020161725607*^9, 
   3.6560201747646165`*^9}, 3.6560202324236164`*^9, {3.6560202971176167`*^9, 
   3.6560203268476167`*^9}, {3.6560203897366166`*^9, 
   3.6560204103176165`*^9}, {3.6560205283466167`*^9, 3.6560205469946165`*^9}, 
   3.6560205972886167`*^9, 3.656020641778617*^9, {3.6560207247346163`*^9, 
   3.6560207963096166`*^9}, 3.6560208747626166`*^9, 3.6560209823486166`*^9, {
   3.6560210187286167`*^9, 3.6560210712776165`*^9}, {3.656021156957617*^9, 
   3.656021210078805*^9}, {3.656021361624958*^9, 3.656021382780073*^9}, {
   3.656021428219617*^9, 3.6560214544602404`*^9}, {3.656021492951089*^9, 
   3.6560215519029837`*^9}, {3.656021598947688*^9, 3.6560216286556587`*^9}, 
   3.6560216596887617`*^9, 3.6560216916379557`*^9, {3.656021821802971*^9, 
   3.656021830101801*^9}, {3.656022111260914*^9, 3.656022167154503*^9}, {
   3.65602222753054*^9, 3.656022233568144*^9}, {3.6560223902988153`*^9, 
   3.656022473043089*^9}, 3.656022800971878*^9, {3.6560660257500477`*^9, 
   3.6560661826121798`*^9}, {3.6560665698269396`*^9, 
   3.6560666592701483`*^9}, {3.6560667511806717`*^9, 
   3.6560668975830297`*^9}, {3.656067165591581*^9, 3.6560671933105807`*^9}, {
   3.656067297286581*^9, 3.656067297923581*^9}, {3.6560674028775806`*^9, 
   3.656067508614581*^9}, {3.6560675548465805`*^9, 3.656067554963581*^9}, {
   3.6560675973091087`*^9, 3.6560677409004664`*^9}, {3.656067814985874*^9, 
   3.6560678829196672`*^9}, {3.6560679281571903`*^9, 3.656067943723747*^9}, {
   3.65606805525298*^9, 3.6560680652059803`*^9}, 3.65606811126698*^9, {
   3.6560692710843763`*^9, 3.656069328175376*^9}, {3.6560693637093763`*^9, 
   3.656069388347376*^9}, 3.6560694820513763`*^9, {3.656069576517376*^9, 
   3.656069591082376*^9}, {3.656069967711144*^9, 3.6560699689932723`*^9}, {
   3.6560700010424767`*^9, 3.6560700103714094`*^9}, {3.6560760677691264`*^9, 
   3.6560761294362926`*^9}, {3.6560761634976983`*^9, 
   3.6560761805294013`*^9}, {3.6560919230513973`*^9, 3.656091923233415*^9}, {
   3.6560922376088495`*^9, 3.6560922404431334`*^9}, {3.656092471267213*^9, 
   3.656092542551341*^9}, {3.6560925760116863`*^9, 3.6560925803161173`*^9}, {
   3.656092646075692*^9, 3.656092649515036*^9}, {3.6560927960536885`*^9, 
   3.65609280466955*^9}, {3.656093002572338*^9, 3.656093027737855*^9}, {
   3.656093105376618*^9, 3.656093119530033*^9}, 3.656093201157195*^9, {
   3.6560935164907255`*^9, 3.656093534458522*^9}, 3.656093593280403*^9, {
   3.6560936357659225`*^9, 3.6560936485649223`*^9}, {3.6560938361719227`*^9, 
   3.6560938487349224`*^9}, {3.6560939367489223`*^9, 3.6560939373849225`*^9}, 
   3.656094001151922*^9, 3.656094036988922*^9, {3.6560940840609226`*^9, 
   3.6560942498219223`*^9}, {3.6560945023869224`*^9, 3.656094537879868*^9}, {
   3.6560945710431843`*^9, 3.6560945753506145`*^9}, {3.6560946394470234`*^9, 
   3.6560946934364223`*^9}, {3.656094799859063*^9, 3.656094903309407*^9}, {
   3.6560949458446603`*^9, 3.6560949511601915`*^9}, 3.6560955879488645`*^9, {
   3.6560957436332836`*^9, 3.6560957497652836`*^9}, {3.6560958306632833`*^9, 
   3.6560958455482836`*^9}, {3.6560959286562834`*^9, 3.6560959516892834`*^9}, 
   3.656096093419284*^9, {3.6560961549132833`*^9, 3.6560961624882836`*^9}, 
   3.6560963758242836`*^9, {3.656100468012304*^9, 3.6561005155343037`*^9}, {
   3.656100563715304*^9, 3.656100573274304*^9}, {3.656100647247304*^9, 
   3.656100671462304*^9}, 3.656100763677304*^9, {3.656100796591304*^9, 
   3.6561008325133038`*^9}, {3.656100883457304*^9, 3.6561008846253037`*^9}, {
   3.656100974960559*^9, 3.6561010119382563`*^9}, {3.656101079257987*^9, 
   3.6561012102660866`*^9}, {3.65610150522258*^9, 3.656101512960353*^9}, {
   3.6561015585639133`*^9, 3.656101621650221*^9}, {3.6561016576358194`*^9, 
   3.6561016596780233`*^9}, 3.656101700300085*^9, {3.6561017582038755`*^9, 
   3.6561017807301273`*^9}, {3.6561019577048235`*^9, 3.656101968302883*^9}, {
   3.6561020528773394`*^9, 3.656102065472599*^9}, {3.6561021409591465`*^9, 
   3.6561021493949904`*^9}, {3.656102844720516*^9, 3.6561029184868917`*^9}, 
   3.656102951219165*^9, {3.656103128460887*^9, 3.656103129950036*^9}, {
   3.6561034941864557`*^9, 3.6561035097930164`*^9}, {3.656103541420179*^9, 
   3.6561035688049173`*^9}, {3.656103627521788*^9, 3.6561036715201874`*^9}, 
   3.6561037643614707`*^9, {3.656103870782112*^9, 3.6561039030766635`*^9}, 
   3.6561039989544845`*^9, {3.6561069455720434`*^9, 3.6561069541425333`*^9}, 
   3.6561070526761694`*^9, {3.656107191492109*^9, 3.65610720444985*^9}, {
   3.6561073142401295`*^9, 3.656107359900741*^9}, {3.6561074190851264`*^9, 
   3.656107480092616*^9}, {3.6561075631283655`*^9, 3.6561075743760085`*^9}, 
   3.656107647809209*^9, {3.656108040948695*^9, 3.6561080659021225`*^9}, {
   3.6561081228493795`*^9, 3.6561081274676437`*^9}, 3.6561082595161963`*^9, {
   3.656108360417968*^9, 3.656108390211672*^9}, {3.6561085197490807`*^9, 
   3.6561085422803698`*^9}, {3.656108577382377*^9, 3.656108690949873*^9}, 
   3.6561087270169363`*^9, 3.6561087719455056`*^9, {3.656108890414282*^9, 
   3.6561089299205413`*^9}, {3.656108967277678*^9, 3.6561089743840847`*^9}, {
   3.6561090145283804`*^9, 3.656109021625787*^9}, {3.656109062574129*^9, 
   3.6561090963130584`*^9}, {3.6561091726494246`*^9, 
   3.6561091878222923`*^9}, {3.6561092309967623`*^9, 3.656109232408843*^9}, 
   3.6561092628605843`*^9, {3.6561093155345974`*^9, 3.6561093389329357`*^9}, {
   3.6561094204796*^9, 3.656109423232757*^9}, {3.6561096729960427`*^9, 
   3.656109677768316*^9}, {3.6561097797661495`*^9, 3.6561100218329954`*^9}, {
   3.656110056044952*^9, 3.656110079463291*^9}, {3.6561101470521574`*^9, 
   3.6561101669812975`*^9}, {3.6561102032673726`*^9, 3.656110214312004*^9}, {
   3.656110246844865*^9, 3.6561102723923264`*^9}, {3.656110310942531*^9, 
   3.6561103129366455`*^9}, {3.7812066415576477`*^9, 3.781206642678727*^9}, {
   3.7812066757354546`*^9, 3.781206686091937*^9}, {3.781206727488229*^9, 
   3.7812067473224897`*^9}, {3.7812067872911973`*^9, 
   3.7812068154930267`*^9}, {3.781208644414544*^9, 3.7812086453992496`*^9}, {
   3.781210621020629*^9, 3.7812106723685303`*^9}, {3.795118040838149*^9, 
   3.7951180411050296`*^9}}],

Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`a$$ = 10, $CellContext`L$$ = 
    1, $CellContext`t$$ = 0.0001, Typeset`show$$ = True, 
    Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`t$$], 0.115, "Angle"}, 0.0001, 0.5235}, {{
       Hold[$CellContext`L$$], 1, "Wavelength"}, 0.5, 2}, {{
       Hold[$CellContext`a$$], 10, "Slit Width"}, 4, 10}}, Typeset`size$$ = {
    1152., {293., 298.}}, Typeset`update$$ = 0, Typeset`initDone$$, 
    Typeset`skipInitDone$$ = False, $CellContext`t$2657$$ = 
    0, $CellContext`L$2658$$ = 0, $CellContext`a$2659$$ = 0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     1, StandardForm, 
      "Variables" :> {$CellContext`a$$ = 10, $CellContext`L$$ = 
        1, $CellContext`t$$ = 0.115}, "ControllerVariables" :> {
        Hold[$CellContext`t$$, $CellContext`t$2657$$, 0], 
        Hold[$CellContext`L$$, $CellContext`L$2658$$, 0], 
        Hold[$CellContext`a$$, $CellContext`a$2659$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> Row[{
         Show[
          
          ListPlot[{{0, -20}, {0, 20}}, Joined -> True, 
           PlotStyle -> {Black, Thick}, Axes -> False, 
           PlotRange -> {{0, 20}, {-10, 10}}, AspectRatio -> 1, ImageSize -> 
           Large, PlotStyle -> PointSize[Medium], PlotLabel -> 
           "Close In View"], 
          
          ListPlot[{{0, (-$CellContext`a$$)/2}, {0, $CellContext`a$$/2}}, 
           Joined -> True, PlotStyle -> {White, Thick}], 
          Graphics[{
            RGBColor[1., 0.8, 0.8], 
            
            Polygon[{{0, (-$CellContext`a$$)/2}, {0, $CellContext`a$$/2}, {
              30, 30 Tan[$CellContext`t$$] + $CellContext`a$$/2}, {
              30, 30 Tan[$CellContext`t$$] - $CellContext`a$$/2}}]}], 
          If[$CellContext`a$$ Sin[$CellContext`t$$] > $CellContext`L$$, 
           Graphics[{
             RGBColor[0.8, 0.6, 0.6], 
             
             Polygon[{{0, $CellContext`a$$/2}, {
               30, 30 Tan[$CellContext`t$$] + $CellContext`a$$/2}, {
               30, 30 Tan[$CellContext`t$$] + $CellContext`a$$/
                 2 - $CellContext`DarkWidth[$CellContext`t$$, \
$CellContext`L$$, $CellContext`a$$]}, {
               0, $CellContext`a$$/
                 2 - $CellContext`DarkWidth[$CellContext`t$$, \
$CellContext`L$$, $CellContext`a$$]}}]}], 
           ListPlot[{{-10, 0}}]], 
          If[
          Mod[$CellContext`a$$ 
             Sin[$CellContext`t$$], $CellContext`L$$] > $CellContext`L$$/2, 
           Graphics[{
             RGBColor[0.8, 0.6, 0.6], 
             
             Polygon[{{0, (-$CellContext`a$$)/2}, {
               30, 30 Tan[$CellContext`t$$] - $CellContext`a$$/2}, {
               30, 30 Tan[$CellContext`t$$] - $CellContext`a$$/
                2 + $CellContext`DarkWidth2[$CellContext`t$$, \
$CellContext`L$$, $CellContext`a$$]}, {
               0, (-$CellContext`a$$)/
                 2 + $CellContext`DarkWidth2[$CellContext`t$$, \
$CellContext`L$$, $CellContext`a$$]}}]}], 
           ListPlot[{{-10, 0}}]], 
          If[
          Mod[$CellContext`a$$ 
             Sin[$CellContext`t$$], $CellContext`L$$] > $CellContext`L$$/2, 
           Graphics[{
             RGBColor[0.8, 0.6, 0.6], 
             
             Polygon[{{
               0, (-$CellContext`a$$)/2 + $CellContext`L$$/(2 
                 Sin[$CellContext`t$$])}, {
               30, 30 Tan[$CellContext`t$$] - $CellContext`a$$/
                2 + $CellContext`L$$/(2 Sin[$CellContext`t$$])}, {
               30, 30 Tan[$CellContext`t$$] - $CellContext`a$$/
                2 + $CellContext`L$$/(2 
                 Sin[$CellContext`t$$]) + \
$CellContext`DarkWidth2[$CellContext`t$$, $CellContext`L$$, \
$CellContext`a$$]}, {
               0, (-$CellContext`a$$)/2 + $CellContext`L$$/(2 
                 Sin[$CellContext`t$$]) + \
$CellContext`DarkWidth2[$CellContext`t$$, $CellContext`L$$, \
$CellContext`a$$]}}]}], 
           ListPlot[{{-10, 0}}]], 
          ListPlot[
           Table[{{0, $CellContext`yoff}, 
             
             RotationTransform[$CellContext`t$$, {0, $CellContext`yoff}][{
              30, $CellContext`yoff}]}, {$CellContext`yoff, $CellContext`a$$/
             2, (-$CellContext`a$$)/2, (-$CellContext`L$$)/(2 
             Sin[$CellContext`t$$])}], Joined -> True, PlotStyle -> {Red}], 
          Table[
           ParametricPlot[
            
            RotationTransform[$CellContext`t$$, {
             0, $CellContext`yoff}][{$CellContext`u, 
              0.4 Sin[(2 
                  Pi) ($CellContext`u/$CellContext`L$$)] + \
$CellContext`yoff}], {$CellContext`u, 0, 5}, PlotStyle -> {Red}, PlotPoints -> 
            Ceiling[20/$CellContext`L$$], MaxRecursion -> 1, 
            PerformanceGoal -> {
             "Speed"}], {$CellContext`yoff, $CellContext`a$$/
            2, (-$CellContext`a$$)/2, (-$CellContext`L$$)/(2 
            Sin[$CellContext`t$$])}], 
          ListPlot[{{0, $CellContext`a$$/2}, 
            
            RotationTransform[$CellContext`t$$, {0, $CellContext`a$$/2}][{
             0, (-$CellContext`a$$)/2}]}, Joined -> True, 
           PlotStyle -> {Dashed, 
             PointSize[Large], Black}], 
          ListPlot[{{0, (-$CellContext`a$$)/2}, 
            
            RotationTransform[$CellContext`t$$, {0, (-$CellContext`a$$)/2}][{
             0, Plus[$CellContext`a$$]/2}]}, Joined -> True, 
           PlotStyle -> {Dashed, 
             PointSize[Large], Black}]], 
         Show[
          
          ListPlot[{{0, -30}, {0, 30}}, Joined -> True, 
           PlotStyle -> {Black, $CellContext`Thicker}, Axes -> False, 
           PlotRange -> {{-10, 190}, {-105, 95}}, AspectRatio -> 1, ImageSize -> 
           Large, PlotStyle -> PointSize[Medium], PlotLabel -> 
           "Far Out View"], 
          
          ListPlot[{{0, (-$CellContext`a$$)/2}, {0, $CellContext`a$$/2}}, 
           Joined -> True, PlotStyle -> {White, Thick}], 
          Graphics[{
            
            RGBColor[{0.8, 0.6, 
              0.6} + {0.2, 0.2, 
               0.2} ($CellContext`Fraunhofer[$CellContext`t$$, \
$CellContext`L$$, $CellContext`a$$]/(15 $CellContext`a$$))], 
            
            Polygon[{{0, (-$CellContext`a$$)/2}, {0, $CellContext`a$$/2}, {
              190, 190 Tan[$CellContext`t$$] + 0.5}, {
              190, 190 Tan[$CellContext`t$$] - 0.5}}]}], 
          
          ParametricPlot[{
           190 - $CellContext`Fraunhofer[$CellContext`tt, $CellContext`L$$, \
$CellContext`a$$], 190 Tan[$CellContext`tt]}, {$CellContext`tt, -0.6, 0.6}, 
           PlotStyle -> {Blue}, PlotPoints -> 100, MaxRecursion -> 1, 
           PerformanceGoal -> {"Speed"}], 
          
          ListPlot[{{
            190 - $CellContext`Fraunhofer[$CellContext`t$$, $CellContext`L$$, \
$CellContext`a$$], 190 Tan[$CellContext`t$$]}, {
            190, 190 Tan[$CellContext`t$$]}}, Joined -> True, Joined -> True, 
           PlotStyle -> {Dashed, Blue}], 
          
          ListPlot[{{
            190 - $CellContext`Fraunhofer[$CellContext`t$$, $CellContext`L$$, \
$CellContext`a$$], 190 Tan[$CellContext`t$$]}}, PlotStyle -> {
             PointSize[Large], Blue}], 
          ListPlot[{{0, 0}, {200, 0}}, Joined -> True, PlotStyle -> {Dashed, 
             PointSize[Small], Black}], 
          
          ListPlot[{{189.8, -100}, {189.8, 1000}}, Joined -> True, 
           PlotStyle -> {Black}]]}], 
      "Specifications" :> {{{$CellContext`t$$, 0.115, "Angle"}, 0.0001, 
         0.5235}, {{$CellContext`L$$, 1, "Wavelength"}, 0.5, 
         2}, {{$CellContext`a$$, 10, "Slit Width"}, 4, 10}}, "Options" :> {}, 
      "DefaultOptions" :> {}],
     ImageSizeCache->{1203., {368., 374.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    Initialization:>({$CellContext`DarkWidth[
         Pattern[$CellContext`t, 
          Blank[]], 
         Pattern[$CellContext`L, 
          Blank[]], 
         Pattern[$CellContext`a, 
          Blank[]]] = ($CellContext`L Csc[$CellContext`t]) 
        Quotient[$CellContext`a, $CellContext`L 
          Csc[$CellContext`t]], $CellContext`DarkWidth2[
         Pattern[$CellContext`t, 
          Blank[]], 
         Pattern[$CellContext`L, 
          Blank[]], 
         Pattern[$CellContext`a, 
          Blank[]]] = 
       Mod[$CellContext`a, ($CellContext`L Csc[$CellContext`t])/
         2], $CellContext`Fraunhofer[
         Pattern[$CellContext`t, 
          Blank[]], 
         Pattern[$CellContext`L, 
          Blank[]], 
         Pattern[$CellContext`a, 
          Blank[]]] = (((15 $CellContext`L^2) Csc[$CellContext`t]^2) 
         Sin[(($CellContext`a Pi) 
             Sin[$CellContext`t])/$CellContext`L]^2)/($CellContext`a Pi^2)}; 
     Typeset`initDone$$ = True),
    SynchronousInitialization->True,
    UndoTrackedVariables:>{Typeset`show$$, Typeset`bookmarkMode$$},
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output",
 CellChangeTimes->{
  3.6561093435191975`*^9, 3.6561094336933556`*^9, {3.6561097945329943`*^9, 
   3.656109805536624*^9}, 3.6561098700543137`*^9, 3.6561099101726084`*^9, {
   3.6561099647207284`*^9, 3.656109988031062*^9}, 3.6561100235210915`*^9, 
   3.6561100807593656`*^9, {3.656110152907492*^9, 3.656110172220597*^9}, 
   3.6561102163281193`*^9, 3.656110275095481*^9, 3.6561103151277704`*^9, 
   3.656158078726777*^9, 3.781206488500765*^9, 3.7812066469737673`*^9, 
   3.781206754808883*^9, 3.7812068634550276`*^9, 3.781208650301484*^9, 
   3.7812106840748844`*^9, 3.7951180587460003`*^9}]
}, {2}]]
},
WindowSize->{1519, 805},
WindowMargins->{{Automatic, -3}, {Automatic, 0}},
PrivateNotebookOptions->{"VersionedStylesheet"->{"Default.nb"[8.] -> False}},
FrontEndVersion->"10.2 for Microsoft Windows (64-bit) (July 6, 2015)",
StyleDefinitions->"Default.nb"
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{}
*)
(*CellTagsIndex
CellTagsIndex->{}
*)
(*NotebookFileOutline
Notebook[{
Cell[CellGroupData[{
Cell[1486, 35, 1512, 34, 19, "Input",
 CellOpen->False,
 InitializationCell->True],
Cell[3001, 71, 483, 14, 19, "Output",
 CellOpen->False]
}, {2}]],
Cell[CellGroupData[{
Cell[3518, 90, 827, 22, 19, "Input",
 CellOpen->False,
 InitializationCell->True],
Cell[4348, 114, 611, 16, 19, "Output",
 CellOpen->False]
}, Closed]],
Cell[CellGroupData[{
Cell[4996, 135, 36068, 751, 786, "Input",
 InitializationCell->True],
Cell[41067, 888, 10039, 214, 759, "Output"]
}, {2}]]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature 2wp1gN0V72mSkAKJT63yJ2wN *)

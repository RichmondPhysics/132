\documentclass[english,twoside]{article}
%labmanual.cls is a modified version of article.cls, tweaked to handle \part{} differently

\input{../../131/StudentGuideModule1/labmanual_formatting_commands} %all general latex packages, commands, and definitions now here.
\newcommand{\coursefolder}{Phys132} %This defines the place students will look for various files
\ForceSectionOddPage %This option makes each lab start on odd numbered page (right hand side).
\externaldocument{master132}

%syntax: \includeonly{lab1,lab2,lab3} with no spaces after the commas.
%\includeonly{biot_savart_law/biot_savart_law, charge_density/charge_density,eoverm/eoverm }
%DANGER: The includeonly statement will make a document that does NOT have sequential page numbers.

%Use the line below to disable hyperlinks, to make sure no markups are visible around references when printing.
\hypersetup{draft}

\newcommand{\supplementmark}{S}

\titleformat{\section}{\normalfont\Large\bfseries}{\supplementmark \thesection}{1em}{}
\fancyhead[LO,RE]{\slshape \rightmark} 
\fancyhead[LE,RO]{\slshape \supplementmark \leftmark} % Reversed LE, RO to  LO,RE to make headers come out correctly on even/odd 

\begin{document}

\setcounter{page}{1}  %Set this to desired first page
\setcounter{section}{0} %set this to desired first section number MINUS ONE
%\includelab{1}{electric_circuits/electric_circuits}
%\includelab{1}{electric_circuits2/electric_circuits2}
\includelab{0}{electric_flux_exercise/electric_flux_exercise}

%--------------------------------------------
%Put include or includelab statements for labs below here.
%\includelab{1}[../../131/StudentGuideModule1/]{check_your_units/check_your_units}
%\includelab{0}{interference/interferenceOfLight} %Jerry's Version
%\includelab{1}{interference_of_light/interference} %Matt's Version
%\includelab{0}{diffraction/diffractionOfLight} %Jerry's Version
%\includelab{1}{diffraction_of_light/diffraction} %Matt's Version


%--------------------------------------------
\startappendix
%\setcounter{section}{12} %set this counter to number MINUS ONE corresponding to desired appendix letter. (4 for `E', etc.)
%Put include statements for supplementary appendices below here.
%\includelab{1}{appendices/digital_multimeters/digital_multimeters}
\includelab{1}{M_problems/M_problems}
%\startinstructornotes
\end{document}
